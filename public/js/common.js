...
function createPostHtml(postData, largeFont = false) {
  ...
  try {
    ...
    return console.log(
      "PostedBy is Used now only in following and followers page "
    );
  }
  ...
  if (isRetweet) {
    retweetText = `<span>
                          ...
                          Retweeted by <a href='/profile/${retweetedBy}'>@<span class='retweeted'>${retweetedBy}</span></a>    
                      </span>`;
  }
  ...
  if (postData.replyTo && postData.replyTo._id) {
    ...
    replyFlag = `<div class='replyFlag'>
      <i class="fas fa-reply"></i> Replying to <a href='/profile/${replyToUsername}'>@${replyToUsername} </a>
                  </div>
      `;
  }
  ...
  if (postData.postedBy._id == userLoggedIn._id) {
    ...
    button = `  <button class= 'pinButton ${pinnedClass}' data-id="${postData._id}" data-toggle="modal" data-target="${dataTarget}"><i class='fas fa-thumbtack'></i></button>
                  <button data-id="${postData._id}" data-toggle="modal" data-target="#deletePostModal" class="del"> <i class='fas fa-times'> </i> </button>`;
  }

  return `<div class='post ${largeFontclass}'  data-id='${postData._id}'>
                  <div class='postActionContainer'>
                   ${retweetText}  ${replyFlag}
                  </div>
...
function outputPosts(results, container) {
  ...
  if (results.length == 0) {
    container.append(
      "<span class='noResults'>Nothing to show 😭. Go back Later</span>"
    );
  }
...
function reloadBefore() {
  window.setTimeout(location.reload(), 1650);
}
